...
int main() {
    ...
    for (int i = 0; i < n; i++) {
        ...
        fgets(students[i].name, MAX_NAME, stdin); //remove newline character from the string
        students[i].name[strcspn(students[i].name, "\n")] = 0;
        printf("Enter score of student %d: ", i + 1);
        scanf("%f", &students[i].score);
        getchar(); 
    }
    qsort(students, n, sizeof(Student), compare); //sort students based on their scores
